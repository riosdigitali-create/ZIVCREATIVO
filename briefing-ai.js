...
(function () {
  ...
  /* -------- PREGUNTAS CORE (siempre aparecen, antes de las dinámicas) -------- */
  const CORE_QUESTIONS = [
    { id: 'nombre_negocio', pregunta: '¿Cómo se llama tu negocio o marca?',
      tipo: 'texto' },
    { id: 'propuesta_unica', pregunta: '¿Qué hace especial a tu negocio? (en una frase: tu diferenciador)',
      tipo: 'texto' },
    { id: 'tono_marca', pregunta: '¿Qué tono visual quieres transmitir?',
      tipo: 'select',
      opciones: ['Minimalista y limpio','Premium y elegante','Cálido y artesanal','Divertido y colorido','Tecnológico y moderno','Editorial y serio','Lujo y exclusivo','Natural y orgánico'] },
    { id: 'paleta_colores', pregunta: '¿Qué paleta de colores prefieres? (ej: "negro y dorado", "tonos tierra")',
      tipo: 'texto' },
    { id: 'secciones', pregunta: '¿Qué secciones necesitas en la landing?',
      tipo: 'checkbox',
      opciones: ['Hero principal','Servicios / Productos','Galería de fotos','Testimonios','Precios / Paquetes','Sobre nosotros','Equipo','FAQ','Blog','Mapa / Ubicación','Formulario de contacto','Reservas online','Tienda / Carrito'] },
    { id: 'cta_principal', pregunta: '¿Cuál es la acción principal que quieres del visitante?',
      tipo: 'select',
      opciones: ['Reservar una cita','Comprar un producto','Llenar formulario de contacto','Llamar por WhatsApp','Suscribirse','Descargar algo (PDF, app)','Visitar la tienda física','Cotizar un servicio'] },
    { id: 'publico_objetivo', pregunta: '¿Quién es tu cliente ideal? (edad, género, intereses, nivel económico)',
      tipo: 'texto' },
  ];

  /* -------- FALLBACK PREGUNTAS GENÉRICAS (si la IA falla totalmente) -------- */
  const FALLBACK_QUESTIONS = CORE_QUESTIONS.slice();
  ...
  /* -------- Llamada 1: briefing (CORE + dinámicas IA) -------- */
  async function fetchBriefing(prompt) {
    // Las preguntas CORE siempre van primero
    const coreQs = CORE_QUESTIONS.slice();

    // Pedimos a Gemini 2-3 preguntas extra ESPECÍFICAS al proyecto
    let aiQs = [];
    try {
      ...
      const qs = Array.isArray(parsed?.preguntas) ? parsed.preguntas : [];
      // Filtrar las que duplican IDs/temas de las CORE
      const coreIds = new Set(coreQs.map(q => q.id.toLowerCase()));
      const coreTopics = ['nombre','color','tono','seccion','cta','accion','publico','diferenciador'];
      aiQs = qs.filter(q => {
        const id = (q.id || '').toLowerCase();
        const txt = (q.pregunta || '').toLowerCase();
        if (coreIds.has(id)) return false;
        if (coreTopics.some(t => id.includes(t))) return false;
        if (coreTopics.some(t => txt.includes(t))) return false;
        return true;
      }).slice(0, 3).map((q, i) => ({
        id: q.id || ('ai_q' + i),
        pregunta: q.pregunta || `Pregunta específica ${i + 1}`,
        tipo: ['texto', 'select', 'checkbox'].includes(q.tipo) ? q.tipo : 'texto',
        opciones: Array.isArray(q.opciones) ? q.opciones : []
      }));
    } catch (e) {
      console.warn('[briefing-ai] /briefing falló o JSON inválido, sólo se usan CORE', e);
      aiQs = [];
    }
  ...
  function showError(stage, msg) {
    const el = document.getElementById('bai-error-' + stage);
    if (!el) return;
    el.textContent = msg;
  ...
  function setLoadingText(html) {
    const el = $('#bai-loading-text');
    if (el) el.innerHTML = html;
  }

  /* -------- Historial UI -------- */
  function renderHistory() {
    const list = $('#bai-history-list');
    if (!list) return;
    const items = getHistory();
    list.innerHTML = '';
    if (!items.length) {
      list.innerHTML = '<div class="bai-history-empty">Aún no has generado ninguna landing.</div>';
      return;
    }
    items.forEach((it, i) => {
      ...
      div.innerHTML = `
        <span class="bai-h-prompt">${escapeHtml(it.prompt || '(sin descripción)')}</span>
        <span class="bai-h-time">${date.toLocaleDateString()} ${String(date.getHours()).padStart(2,'0')}:${String(date.getMinutes()).padStart(2,'0')}</span>
      `;
      div.addEventListener('click', () => {
        state.html = it.html;
        state.prompt = it.prompt || '';
        renderPreview(it.html);
        setStep(3);
        showStage('bai-stage-3');
      });
  ...
  /* -------- CTA con precio del cotizador + WhatsApp -------- */
  function updateCTA() {
    const biz = detectBusiness(state.prompt, state.answers);
    const price = biz.price;
    const compare = compareAgency(price);
    const save = compare - price;
    const priceEl = $('#bai-cta-price');
    const compareEl = $('#bai-cta-compare');
    const saveEl = $('#bai-cta-save');
    if (priceEl) priceEl.textContent = fmtPrice(price);
    if (compareEl) compareEl.textContent = fmtPrice(compare);
    if (saveEl) saveEl.textContent = fmtPrice(save);

    // Mensaje WhatsApp del CTA principal
    const promptSummary = (state.prompt || '').slice(0, 120);
    const ctaMsg = `Hola Ismael, generé un borrador con tu IA para mi proyecto: "${promptSummary}". Quiero la "${biz.title}" por $${fmtPrice(price)} MXN (pago único). ¿Cuándo podemos platicar?`;
    const ctaWa = $('#bai-cta-wa');
    if (ctaWa) ctaWa.href = buildWhatsAppLink(ctaMsg);

    // Guardar para uso del chat
    state._biz = biz;
    state._price = price;
  }
  ...
  function appendChat(role, text) {
    ...
    const log = $('#bai-chat-log');
    const div = document.createElement('div');
    div.className = 'bai-chat-msg ' + role;
  ...
  function attachHandlers() {
    ...
    // Counter
    const ta = $('#bai-prompt');
    const counter = $('#bai-counter');
    ta.addEventListener('input', () => {
      const n = ta.value.length;
      counter.textContent = `${n} / 600`;
      counter.classList.toggle('error', n > 0 && n < MIN_CHARS);
    });

    // Historial toggle
    $('#bai-history-btn').addEventListener('click', () => {
      const h = $('#bai-history');
      h.classList.toggle('open');
      if (h.classList.contains('open')) renderHistory();
    });

    // Stage 1 -> Stage 2 (briefing)
    $('#bai-go-briefing').addEventListener('click', async () => {
      clearError(1);
      const txt = ta.value.trim();
      if (txt.length < MIN_CHARS) {
        showError(1, `Escribe al menos ${MIN_CHARS} caracteres para que la IA pueda entender la idea.`);
        return;
      }
      state.prompt = txt;
      setLoadingText('Generando preguntas inteligentes… <strong>aguanta</strong>');
      showStage('bai-stage-loading');
      try {
        const qs = await fetchBriefing(txt);
        state.questions = qs;
        renderQuestions(qs);
        setStep(2);
        showStage('bai-stage-2');
      } catch (err) {
        console.error(err);
        showStage('bai-stage-1');
        showError(1, err.message || 'Error generando el briefing. Intenta de nuevo.');
      }
    });

    // Stage 2 -> Stage 1
    $('#bai-back-1').addEventListener('click', () => {
      setStep(1);
      showStage('bai-stage-1');
    });

    // Stage 2 -> Stage 3 (generate)
    $('#bai-go-generate').addEventListener('click', async () => {
      clearError(2);
      const ans = collectAnswers(state.questions);
      state.answers = ans;
      // Construir prompt MUY enriquecido para que Gemini tenga máximo contexto
      const biz = detectBusiness(state.prompt, ans);
      const lines = [
        `PROYECTO: ${state.prompt}`,
        `TIPO DE NEGOCIO DETECTADO: ${biz.name} (template "${biz.title}")`,
        ``,
        `BRIEFING DETALLADO DEL CLIENTE:`,
      ];
      state.questions.forEach(q => {
        const v = ans[q.id];
        const txt = Array.isArray(v) ? v.join(', ') : v;
        if (txt && String(txt).trim()) lines.push(`• ${q.pregunta}\n  → ${txt}`);
      });
      lines.push('');
      lines.push('INSTRUCCIONES ADICIONALES:');
      lines.push('- Usa el NOMBRE EXACTO del negocio que dio el cliente como marca principal en hero, header, footer y title.');
      lines.push('- Aplica la PALETA DE COLORES que pidió, traduciéndola a Tailwind classes (bg-amber-900, text-stone-100, etc.) o a colores hex inline.');
      lines.push('- Solo crea las SECCIONES que marcó el cliente — ni más, ni menos.');
      lines.push('- El CTA principal debe ser el más visible y repetirse 2-3 veces a lo largo de la página.');
      lines.push('- El TONO de la copy debe coincidir con el tono visual elegido.');
      lines.push('- Habla directamente al PÚBLICO OBJETIVO con sus palabras y nivel de lenguaje.');
      lines.push('- La PROPUESTA ÚNICA debe estar destacada en el hero como el headline principal.');
      const enriched = lines.join('\n');
      setLoadingText('Generando tu landing… <strong>esto tarda 20-40s</strong>');
      showStage('bai-stage-loading');
      try {
        const html = await fetchHtml(enriched);
        if (!html || html.length < 50) throw new Error('La IA devolvió una respuesta vacía. Intenta de nuevo.');
        state.html = html;
        saveLast(html, state.prompt);
        pushHistory(html, state.prompt);
        renderPreview(html);
        updateCTA();
        setStep(3);
        showStage('bai-stage-3');
        ...
        console.error(err);
        showStage('bai-stage-2');
        showError(2, err.message || 'Error generando el HTML. Intenta de nuevo.');
      }
    });
    ...
    $('#bai-download').addEventListener('click', () => {
      ...
      const name = (state.prompt || 'landing').slice(0, 40).replace(/[^a-z0-9]+/gi, '-').toLowerCase();
      a.href = url;
      a.download = `${name || 'landing'}-ziv.html`;
    ...
    $('#bai-restart').addEventListener('click', () => {
      state = { prompt: '', questions: [], answers: {}, html: '', chatLog: [] };
      ta.value = '';
      counter.textContent = '0 / 600';
      $('#bai-chat-log').innerHTML = '';
      setStep(1);
      showStage('bai-stage-1');
    });

    // Chat de cambios → abre WhatsApp directo (sin IA)
    const sendToWhatsApp = () => {
      const inp = $('#bai-chat-input');
      const txt = inp.value.trim();
      if (!txt) {
        inp.focus();
        inp.placeholder = '⚠ Escribe primero qué quieres cambiar';
        return;
      }
      const biz = state._biz || detectBusiness(state.prompt, state.answers);
      const price = state._price || (biz && biz.price) || 1949;
      const promptSummary = (state.prompt || '').slice(0, 120);
      const message =
        `Hola Ismael, generé un borrador en tu sitio para mi proyecto: "${promptSummary}".\n\n` +
        `Quiero la "${biz.title}" (~$${fmtPrice(price)} MXN) pero con estos cambios específicos:\n` +
        `→ ${txt}\n\n` +
        `¿Cuándo podemos platicar?`;
      window.open(buildWhatsAppLink(message), '_blank', 'noopener');
    };
    $('#bai-chat-send').addEventListener('click', sendToWhatsApp);
    $('#bai-chat-input').addEventListener('keydown', (e) => { if (e.key === 'Enter') sendToWhatsApp(); });
  }
  ...
  function openModal() {
    ...
    $('#bai-stage-1').style.display = 'block';
  }
  function closeModal() {
    const bd = $('#bai-backdrop');
  ...
  function init() {
    if (typeof OpenAI === 'undefined') {
      console.warn('[BAI] OpenAI SDK not found. Chat disabled.');
      state.noOpenAI = true;
    }
    // Inserta el modal en el DOM (si aún no existe) antes de enlazar handlers
    if (!document.getElementById('bai-backdrop')) {
  ...
  // Esperar a que el DOM esté listo antes de inyectar el modal

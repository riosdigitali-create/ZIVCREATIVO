...
(function () {
  ...
  const WORKER_URL = window.BAI_WORKER_URL || 'https://briefing-ai.isma-rhisma94.workers.dev';
  ...
  function extractHTML(text){
    if (!text) return '';
    // Si vino en bloque markdown ```html ... ``` lo limpiamos
  ...
  function openModal(){
    ...
    // Si tenemos HTML previo, vamos directo a preview; si no, prompt.
    if (state.html){
  ...
  function renderPreview(html){
    ...
    if (!iframe) return;
    iframe.srcdoc = html;
    // WhatsApp link con resumen del prompt
    const wa = $('#bai-cta-wa');
    if (wa){
  ...
  async function doGenerate(prompt){
    ...
    showStage('loading');
    try {
      // Prompt enriquecido para Gemini: pide HTML completo
      const enriched =
  ...
  window.BAI_open = function(text){
    if (text && typeof text === 'string'){
      // Si el modal aún no existe (por timing), lo asegura
      if (!document.getElementById('bai-backdrop')) init();
      const ta = document.getElementById('bai-prompt');
      if (ta) ta.value = text;
      state.prompt = text;
      openModal();
      // Disparar generación automática si vino con texto
